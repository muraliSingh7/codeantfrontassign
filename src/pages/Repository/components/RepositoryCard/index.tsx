...
interface RepositoryCardProps extends RepositoryInfo {
    className?: string;
}
const DotIcon: React.FC = () => (
    <svg className='dot-icon' viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const DatabaseIcon: React.FC = () => (
    <svg className='database-icon' viewBox="0 0 11 12" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const RepositoryCard: React.FC<RepositoryCardProps> = ({ title, isPublic, size, language, updated, className }) => {
    return (
        <div className={`card-container ${className ? className :''}`}>
            ...
            <div className="info-container">
                ...
                <span className="info-item"><DatabaseIcon /> {size}</span>
                <span className="info-item">{updated}</span>
            </div>
        </div>
    );
};
